...
#corona <- readRDS('~/Dropbox/Joan Barcelo/Present/NYUAD Assistant Professor/Research/Papers/Work in Progress/West European Politics Corona Article/WEP_analysis/data/CoronaNet/coronanet_internal_sub_clean_Aug14.RDS')
...
#corona <- readRDS('~/Dropbox/Joan Barcelo/Present/NYUAD Assistant Professor/Research/Papers/Work in Progress/West European Politics Corona Article/WEP_analysis/data/CoronaNet/coronanet_internal_sub_clean_old.RDS')
...
subnational_data$region[which(subnational_data$region == 'Lombardia')] <- 'Lombardy'
subnational_data$region[which(subnational_data$region == 'Auvergne-Rhône-Alpes')] <- 'Auvergne-Rhone-Alpes'
subnational_data$region[which(subnational_data$region == 'Corse')] <- 'Corsica'
subnational_data$region[which(subnational_data$region == 'Île-de-France')] <- 'Ile-de-France'
corona_sel$target_province[which(corona_sel$target_province == 'Latium')] <- 'Lazio'
subnational_data$region[which(subnational_data$region == 'Normandie')] <- 'Normandy'
subnational_data$region[which(subnational_data$region == 'Piemonte')] <- 'Piedmont'
subnational_data$region[which(subnational_data$region == 'Sicilia')] <- 'Sicily'
subnational_data$region[which(subnational_data$region == 'Sardinia')] <- 'Sardegna'
subnational_data$region[which(subnational_data$region == 'Aosta Valley')] <- "Valle d'Aosta"
subnational_data$region[which(subnational_data$region == 'Apulia')] <- "Puglia"
subnational_data$region[which(subnational_data$region == 'Marche')] <- "Marches"
subnational_data$region[which(subnational_data$region == 'Toscana')] <- "Tuscany"
subnational_data$region[which(subnational_data$region == 'Bretagne')] <- "Brittany"
subnational_data$region[which(subnational_data$region == 'Sicilia')] <- "Sicily"
subnational_data$region[which(subnational_data$region == 'Basilicata')] <- "Basilicate"
...
corona_sel[which(corona_sel$target_province == "Baden"),]$target_province <- rep("Baden-Wuerttemberg", length(corona_sel[which(corona_sel$target_province == "Baden"),]$target_province))
...
corona_sel[which(corona_sel$type == 'Closure and Regulation of Schools' & corona_sel$target_province == "Liguria"),]$date_end <- rep(as.Date("2020-03-04"), length(corona_sel[which(corona_sel$type == 'Closure and Regulation of Schools' & corona_sel$target_province == "Liguria"),]$date_end))
