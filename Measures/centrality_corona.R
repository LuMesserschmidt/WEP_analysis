...
cor(policyCentralization[, c('subGraphStd','centDegStd')])
sub_data %>% filter(country == 'Switzerland',
                    type == 'Closure and Regulation of Schools',
                    date == '2020-03-13') %>% select(policy_id)


data_long %>% filter(country == 'Switzerland',
                    type == 'Closure and Regulation of Schools',
                    date == '2020-03-13')  %>% select(target_province) %>% table



data_long %>% filter(country == 'Switzerland',
                     type == 'Closure and Regulation of Schools',
                     date == '2020-03-13')  %>% select(gov) %>% table
...
data_long %>% filter(country == 'Switzerland',
                     type == 'Closure and Regulation of Schools',
                     date == '2020-03-13' & 
                       init_country_level == 'National')  %>% select(policy_id)  


qualtrics %>% filter()

qualtrics %>% filter(policy_id %in% c(2735572)) %>% data.frame()
